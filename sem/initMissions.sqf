...
SEM_MissionTimerMin = 2;	// Minimum minutes between missions.
SEM_MissionTimerMax = 5;	// Maximum minutes between missions.
...
SEM_AIdisableSniperDamage = false;	// Set to false to allow sniper damage from any distance.
SEM_AIsniperDamageDistance = 1000;	// Max. distance (in meters) where AI takes damage (min. 300 -  max. 1000).
...
_staticMissions = [
	["supplyVanCrash",	"Caminhão de Suprimentos",		45,	100],
	["bPlaneCrash",		"C-130 Destruido",	45,	90],
	["bHeliCrash",		"Ghost Hawk Destruido",	45,	85],
	["bCamp",			"Acampamento de Bandido",		90,	80],
	["bDevice",			"Dispositivo Encontrado",	45,	75],
	/* example */
...
{if !(_x in SEM_removeWeaponsFromDeadAI)then{SEM_removeWeaponsFromDeadAI pushBack _x}}forEach ["launch_RPG32_F"];
{if !(_x in SEM_removeMagazinesFromDeadAI)then{SEM_removeMagazinesFromDeadAI pushBack _x}}forEach ["RPG32_F"];
